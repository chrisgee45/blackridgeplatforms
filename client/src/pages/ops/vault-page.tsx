/**
 * BlackRidge Vault — the platform's own third-party accounts
 * (Railway, Resend, Anthropic, Stripe, AWS, etc.). Same shape as
 * the per-project Accounts tab but unscoped: one global list.
 *
 * Backed by /api/ops/blackridge/service-accounts. Secrets share the
 * same AES-256-GCM master key (ACCOUNT_SECRETS_KEY) as the project
 * vault, so one env var unlocks both.
 */
...
const SERVICE_PRESETS: { value: string; label: string; loginUrl?: string }[] = [
  ...
  { value: "google_pagespeed", label: "Google PageSpeed", loginUrl: "https://console.cloud.google.com/" },
  { value: "hunter", label: "Hunter.io", loginUrl: "https://hunter.io/" },
  { value: "zerobounce", label: "ZeroBounce", loginUrl: "https://zerobounce.net/" },
  { value: "namecheap", label: "Namecheap", loginUrl: "https://namecheap.com/" },
  { value: "plaid", label: "Plaid", loginUrl: "https://dashboard.plaid.com/" },
  { value: "github", label: "GitHub", loginUrl: "https://github.com/" },
...
export default function VaultPage() {
  ...
  const { data: accounts = [], isLoading } = useQuery<ServiceAccount[]>({
    queryKey: ["/api/ops/blackridge/service-accounts"],
  });

  const createMutation = useMutation({
    mutationFn: (payload: any) =>
      apiRequest("POST", "/api/ops/blackridge/service-accounts", payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/ops/blackridge/service-accounts"] });
      setEditing(null);
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: any }) =>
      apiRequest("PATCH", `/api/ops/blackridge/service-accounts/${id}`, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/ops/blackridge/service-accounts"] });
      setEditing(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiRequest("DELETE", `/api/ops/blackridge/service-accounts/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/ops/blackridge/service-accounts"] });
  ...
  async function handleReveal(accountId: string) {
    ...
    try {
      const res = await apiRequest("POST", `/api/ops/blackridge/service-accounts/${accountId}/reveal`, {});
  ...
  function copyToClipboard(value: string) {
    try { navigator.clipboard.writeText(value); toast({ title: "Copied" }); } catch { /* */ }
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            BlackRidge's third-party accounts — Railway, Resend, Anthropic, Stripe, AWS, etc.
            Secrets are encrypted at rest with AES-256-GCM.
          </p>
        </div>
        <Button onClick={openCreate} data-testid="button-add-blackridge-account">
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="py-10 text-center text-sm text-muted-foreground">
            No accounts tracked yet. Click "Add account" to start.
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-2">
          {accounts.map(a => {
            ...
            return (
              <Card key={a.id} data-testid={`row-blackridge-account-${a.id}`}>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {a.label && (
                          <span className="text-xs px-2 py-0.5 rounded-full bg-muted text-muted-foreground">{a.label}</span>
                        )}
                        {a.hasSecrets && (
                          <span className="text-xs px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-200 border border-blue-500/30">secrets stored</span>
                        )}
                      </div>
                      <div className="mt-1.5 text-sm space-y-0.5">
                        {a.accountEmail && (
                          <div className="flex items-center gap-2 text-muted-foreground">
                            ...
                            <button onClick={() => copyToClipboard(a.accountEmail!)} className="text-muted-foreground hover:text-foreground" title="Copy">
                        ...
                        {a.accountId && (
                          <div className="flex items-center gap-2 text-muted-foreground">
                            ...
                            <button onClick={() => copyToClipboard(a.accountId!)} className="text-muted-foreground hover:text-foreground" title="Copy">
                              <Copy className="w-3 h-3" />
                            </button>
                          </div>
                        )}
                        {a.loginUrl && (
                          <div className="flex items-center gap-2 text-muted-foreground">
                            ...
                            <a href={a.loginUrl} target="_blank" rel="noreferrer" className="text-blue-300 hover:underline inline-flex items-center gap-1">
                              {a.loginUrl}
                        ...
                        {a.notes && (
                          <div className="text-muted-foreground whitespace-pre-wrap mt-1 text-xs">{a.notes}</div>
                        )}
                      </div>
                          ...
                          {Object.keys(revealedSecrets).length === 0 ? (
                            ...
                            Object.entries(revealedSecrets).map(([k, v]) => (
                              <div key={k} className="flex items-center gap-2 text-sm">
                                <span className="text-xs uppercase tracking-wide text-muted-foreground w-32 shrink-0">{k}</span>
                                <span className="font-mono break-all flex-1">{String(v)}</span>
                                <button onClick={() => copyToClipboard(String(v))} className="text-muted-foreground hover:text-foreground" title="Copy">
                                  <Copy className="w-3 h-3" />
                                </button>
                              </div>
                            ))
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      {a.hasSecrets && (
                        <Button size="sm" variant="ghost" onClick={() => handleReveal(a.id)} title={isRevealed ? "Hide" : "Reveal"}>
