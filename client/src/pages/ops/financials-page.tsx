...
import {
  DollarSign, TrendingUp, TrendingDown, Wallet, ChevronDown, ChevronRight, Search, Plus, Loader2, Ban, ArrowUpRight, FileDown, FileText,
} from "lucide-react";
...
import GaapManagementTab from "./gaap-management-tab";
import BankSyncTab from "./bank-sync-tab";
import {
...
export default function FinancialsPage() {
  ...
  const cashBalance = balanceData?.assets?.find(a => a.accountName?.toLowerCase().includes("cash"))?.balance ?? 0;
  ...
  const payoutMutation = useMutation({
    mutationFn: () => apiRequest("POST", "/api/accounting/stripe-payout", {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Dialog open={payoutDialogOpen} onOpenChange={setPayoutDialogOpen}>
            <DialogTrigger asChild>
              <Button size="sm" variant="outline" data-testid="button-stripe-payout">
                <ArrowUpRight className="w-4 h-4 mr-1" /> Stripe Payout
              </Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Record Stripe Payout</DialogTitle>
              </DialogHeader>
              <p className="text-sm text-muted-foreground">
              ...
              <div className="space-y-4 pt-2">
                ...
                <div>
                  <label className="text-sm font-medium">Memo (optional)</label>
                  <Input
                    placeholder="e.g. Stripe payout 2026-05-23"
                    value={payoutMemo}
                    onChange={(e) => setPayoutMemo(e.target.value)}
                    data-testid="input-payout-memo"
                  />
                </div>
                <Button
                  className="w-full"
                  disabled={!payoutAmount || isNaN(parseFloat(payoutAmount)) || parseFloat(payoutAmount) <= 0 || payoutMutation.isPending}
          ...
          <Button
            ...
            onClick={() => {
              if (incomeData && balanceData && cashFlowData) {
                generateFullAccountingPdf(
                  incomeData,
                  balanceData,
                  cashFlowData,
                  (journalEntries ?? []).map((e) => ({
                    ...e,
                    totalDebits: e.totalDebits ?? 0,
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <div className="flex items-center gap-1">
          <div className="overflow-x-auto -mx-3 px-3 sm:mx-0 sm:px-0 scrollbar-hide">
            <TabsList className="inline-flex w-max" data-testid="tabs-financials">
              <TabsTrigger value="income-statement" data-testid="tab-income-statement" className="text-xs sm:text-sm">P&L</TabsTrigger>
              <TabsTrigger value="balance-sheet" data-testid="tab-balance-sheet" className="text-xs sm:text-sm">Balance</TabsTrigger>
              <TabsTrigger value="cash-flow" data-testid="tab-cash-flow" className="text-xs sm:text-sm">Cash Flow</TabsTrigger>
              <TabsTrigger value="general-ledger" data-testid="tab-general-ledger" className="text-xs sm:text-sm">Ledger</TabsTrigger>
              <TabsTrigger value="reconciliation" data-testid="tab-reconciliation" className="text-xs sm:text-sm">Recon</TabsTrigger>
              <TabsTrigger value="bills" data-testid="tab-bills" className="text-xs sm:text-sm">Bills</TabsTrigger>
              <TabsTrigger value="opening-balances" data-testid="tab-opening-balances" className="text-xs sm:text-sm">Opening</TabsTrigger>
              <TabsTrigger value="budget" data-testid="tab-budget" className="text-xs sm:text-sm">Budget</TabsTrigger>
              <TabsTrigger value="gaap" data-testid="tab-gaap" className="text-xs sm:text-sm">GAAP</TabsTrigger>
              <TabsTrigger value="bank-sync" data-testid="tab-bank-sync" className="text-xs sm:text-sm">Bank Sync</TabsTrigger>
            </TabsList>
          </div>
          <HelpTooltip {...HELP_CONTENT[({
            ...
            "budget": "budget",
            "gaap": "gaapPeriods",
          } as Record<string, keyof typeof HELP_CONTENT>)[activeTab] ?? "incomeStatement"]} />
        </div>
        ...
        <TabsContent value="gaap" className="mt-4 space-y-4">
          <GaapManagementTab />
        </TabsContent>
        <TabsContent value="bank-sync" className="mt-4 space-y-4">
          <BankSyncTab />
        </TabsContent>
      </Tabs>
    </div>
...
function GeneralLedgerTab({ entries, loading }: { entries: JournalEntry[]; loading: boolean }) {
  ...
  const voidMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await apiRequest("POST", `/api/ops/transactions/${id}/void`);
      return res.json();
    },
    onSuccess: () => {
      toast({ title: "Entry voided", description: "A reversing journal entry has been created." });
      queryClient.invalidateQueries({ queryKey: ["/api/accounting/income-statement"] });
      queryClient.invalidateQueries({ queryKey: ["/api/accounting/balance-sheet"] });
      queryClient.invalidateQueries({ queryKey: ["/api/accounting/cash-flow"] });
      queryClient.invalidateQueries({ queryKey: ["/api/accounting/transactions"] });
    ...
    onError: (err: Error) => {
      toast({ title: "Cannot void entry", description: err.message, variant: "destructive" });
    },
  });
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="hidden sm:table-cell">Client / Project</TableHead>
              <TableHead className="hidden md:table-cell">Source</TableHead>
              <TableHead className="text-right whitespace-nowrap">Debits</TableHead>
              ...
              filtered.map((entry) => {
                ...
                return (
                  <JournalRow
                    ...
                    onDelete={(id) => {
                      if (window.confirm("Void this journal entry? A reversing entry will be created. This cannot be undone.")) {
  ...
  return (
    <>
      ...
      >
        ...
        <TableCell className="max-w-[120px] sm:max-w-[200px] truncate">{entry.memo || "—"}</TableCell>
        <TableCell className="hidden sm:table-cell max-w-[180px]">
          {entry.clientName || entry.projectName ? (
            <div className="text-sm space-y-0.5">
              {entry.clientName && (
                <div className="font-medium truncate" data-testid={`text-client-${entry.id}`}>{entry.clientName}</div>
              )}
              {entry.projectName && (
                <div className="text-muted-foreground text-xs truncate" data-testid={`text-project-${entry.id}`}>{entry.projectName}</div>
              )}
            </div>
          ) : (
            ...
            {entry.source ? (
              <Badge
                ...
                className={`no-default-hover-elevate no-default-active-elevate text-xs ${
                  entry.source === "reversal" ? "bg-red-100 text-red-700" :
                  entry.source === "adjusting" ? "bg-amber-100 text-amber-700" :
                  entry.source === "year_end_close" ? "bg-purple-100 text-purple-700" : ""
                }`}
              >
                {entry.source}
              </Badge>
        ...
        <TableCell className="text-right">
          <Button
            ...
            data-testid={`button-void-journal-${entry.id}`}
            title="Void entry"
          >
        ...
        <TableRow data-testid={`row-journal-lines-${entry.id}`}>
          <TableCell colSpan={9} className="bg-muted/30 p-0">
            <div className="p-4 space-y-3">
              {(entry.clientName || entry.projectName || entry.paymentLabel) && (
                <div className="flex flex-wrap gap-4 text-sm pb-2 border-b border-border/50">
                  {entry.clientName && (
                    <div data-testid={`detail-client-${entry.id}`}>
                      <span className="text-muted-foreground">Client:</span>{" "}
                      <span className="font-medium">{entry.clientName}</span>
                    </div>
                  )}
                  {entry.projectName && (
                    <div data-testid={`detail-project-${entry.id}`}>
